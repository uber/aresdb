...
type UpsertBatchBuilder interface {
	...
	SetTableSchema(namespace string, table metaCom.Table) (*TableSchema, error)
	...
	PrepareUpsertBatch(namespace, tableName string, columnNames []string, updateModes []memCom.ColumnUpdateMode, rows []Row) ([]byte, int, error)
}
...
// PostEnumCasesFn posts an enum column's strings of the table
type PostEnumCasesFn func(namespace, tableName, columnName string, enumCasesBytes []byte) ([]int, error)

// GetEnumDictFn gets an enum column's dictionary of the table
type GetEnumDictFn func(namespace, tableName, columnName string) ([]string, error)
...
type UpsertBatchBuilderImpl struct {
	...
	// postEnumCasesFn is function to post enum cases
	postEnumCasesFn PostEnumCasesFn

	// getEnumDictFn is function to get enum dictionary
	getEnumDictFn GetEnumDictFn
}
...
// NewUpsertBatchBuilderImpl returns UpsertBatchBuilderImpl
func NewUpsertBatchBuilderImpl(cfg ConnectorConfig, logger *zap.SugaredLogger, metricScope tally.Scope,
	schemas map[string]*TableSchema,
	enumMappings map[string]map[int]EnumDict,
	enumDefaultValueMappings map[string]map[int]int,
	postEnumCasesFn PostEnumCasesFn, getEnumDictFn GetEnumDictFn) UpsertBatchBuilder {
		return &UpsertBatchBuilderImpl {
			...
			enumDefaultValueMappings: enumDefaultValueMappings,
			postEnumCasesFn: postEnumCasesFn,
			getEnumDictFn: getEnumDictFn,
		}
}
...
func (cfg ConnectorConfig) NewConnector(logger *zap.SugaredLogger, metricScope tally.Scope) (Connector, error) {
	...
	connector.postEnumCasesFn = connector.postEnumCases
	connector.getEnumDictFn = connector.fetchEnumDict
...
func (c *connector) fetchEnumDict(namespace, tableName, columnName string) ([]string, error) {
...
func (c *connector) fetchTableSchema(tableName string) (*TableSchema, error) {
	...
	return c.SetTableSchema("", table)
}

func (c *connector) postEnumCases(namespace, tableName, columnName string, enumCasesBytes []byte) ([]int, error) {
	var enumIDs []int
...
func (u *UpsertBatchBuilderImpl) SetTableSchema(namespace string, table metaCom.Table) (*TableSchema, error) {
	enumMappings, enumDefaultValueMappings, err := u.fetchEnumDicts("", table)
	if err != nil {
		return nil, err
	}
...
// PrepareUpsertBatch prepares the upsert batch for upsert,
// returns upsertBatch byte array, number of rows in upsert batch and error.
func (u *UpsertBatchBuilderImpl) PrepareUpsertBatch(namespace, tableName string, columnNames []string,
	...
	for colIndex, columnName := range columnNames {
		...
		if column.IsEnumColumn() {
			if err = u.prepareEnumCases(namespace, tableName, columnName, colIndex, columnID, rows, abandonRows, column.CaseInsensitive, column.DisableAutoExpand); err != nil {
				return nil, 0, err
			}
		}
	}
...
func (u *UpsertBatchBuilderImpl) prepareEnumCases(namespace, tableName, columnName string, colIndex, columnID int, rows []Row, abandonRows map[int]interface{}, caseInsensitive bool, disableAutoExpand bool) error {
	...
	for rowIndex, row := range rows {
		...
		if enumCase, ok := value.(string); ok {
			...
			if _, valueExist := u.enumMappings[tableName][columnID][convertedEnumCase]; !valueExist {
	...
	return u.extendEnumDict(namespace, tableName, columnName, columnID, newEnumCases, caseInsensitive)
}

func (u *UpsertBatchBuilderImpl) extendEnumDict(namespace, tableName, columnName string, columnID int, enumCases []string, caseInsensitive bool) error {
	if len(enumCases) == 0 {
		return nil
	}
	...
	enumIDs, err := u.postEnumCasesFn(namespace, tableName, columnName, enumCasesBytes)
	if err != nil {
		return err
	}
	...
	for index, enumCase := range enumCases {
		...
		u.enumMappings[tableName][columnID][enumCase] = enumIDs[index]
	}
	u.Unlock()
...
func (u *UpsertBatchBuilderImpl) fetchEnumDicts(namespace string, table metaCom.Table) (map[int]EnumDict, map[int]int, error) {
	...
	for columnID, column := range table.Columns {
		...
		if column.IsEnumColumn() && column.DisableAutoExpand {
			enumCases, err := u.getEnumDictFn(namespace, table.Name, column.Name)
			if err == nil {
				...
				u.metricScope.Tagged(map[string]string{
					"table":    table.Name,
					"columnID": strconv.Itoa(columnID),
